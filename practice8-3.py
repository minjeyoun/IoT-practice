...
@app.route('/sub1')  # '/sub1' 경로에 대한 라우트를 정의합니다.
def sub1():  # '/sub1' URL에 접속할 때 실행될 함수입니다.
    return 'SUB1 Page'  # 클라이언트에게 'SUB1 Page'라는 문자열을 반환합니다.

@app.route('/sub2')  # '/sub2' 경로에 대한 라우트를 정의합니다.
def sub2():  # '/sub2' URL에 접속할 때 실행될 함수입니다.
    return 'SUB2 Page'  # 클라이언트에게 'SUB2 Page'라는 문자열을 반환합니다.
